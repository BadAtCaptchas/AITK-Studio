...
Use Node 22.12+, 24 or 26; CI covers Node 22/24/26 across Linux and Windows. Use the reference Python 3.12 environment (3.11 for DGX). Run `python scripts/install_runtime.py --profile <profile>` inside a virtual environment to resolve Torch, TorchCodec and application dependencies together. `--dry-run --report resolution.json` performs fresh resolution without changing the environment. Profiles are legacy-cu128, blackwell-cu128, blackwell-cu130, dgx-cu130 and macos. Do not follow installation with unpinned Torch/Transformers upgrades.
