...
Qwen Image 2.1 supports text-to-image generation and image editing through the `qwen_image_2` architecture. New Studio configurations load the official Qwen weights, then quantize the transformer and text encoder locally to ConvRot int8. The [official model card](https://huggingface.co/Qwen/Qwen-Image-2.1) and [reference repository](https://github.com/QwenLM/Qwen-Image-2.1) describe the model's capabilities.

## Weight sources and precision

The default source is `name_or_path: Qwen/Qwen-Image-2.1`, with `model_kwargs.use_comfy_weights: false`. This loads the official checkpoint directly, including when Comfy repacks are already cached. ConvRot int8 quantization, low-VRAM mode, guidance, resolution, and step presets are unchanged. Disable `quantize` or `quantize_te` to keep the corresponding component in full precision.

Existing saved configurations keep their explicit values. To switch an existing job to the official source, set both `name_or_path: Qwen/Qwen-Image-2.1` and `model_kwargs.use_comfy_weights: false`. To opt into Comfy repacks, set `model_kwargs.use_comfy_weights: true`; `name_or_path: Comfy-Org/Qwen-Image-2.1` remains supported, and older jobs explicitly naming that repo without the flag retain their Comfy source. Explicit checkpoint file paths keep their existing source semantics.

When Comfy substitution is enabled, the transformer and text encoder support both BF16 and ConvRot int8 repacks. With `quantize: false` or `quantize_te: false`, the corresponding component selects its BF16 source rather than reusing cached int8 weights and dequantizing them. If only the int8 file is cached, Studio downloads the BF16 file; existing cached weights remain intact. With quantization enabled, source selection prefers an available cached variant, ranks ConvRot int8 first for `convrot8`, and applies the requested quantization after loading when needed. Text encoder source selection respects `qtype_te`; its vision patch is installed before post-load quantization and device placement.
