...
Stem-only/separation training is disabled because it is not reliable in this upstream
revision. ``model_kwargs.do_separation: true`` is rejected before loading models.
The MelBandRoformer utilities remain available for captioning and standalone processing.
...
class YuE2AudioModel(BaseAudioModel):
    ...
    def __init__(self, device, model_config, dtype="bf16", custom_pipeline=None, noise_scheduler=None, **kwargs):
        ...
        if self.do_separation:
            raise ValueError("YuE2 stem-only training is not supported yet; set do_separation to false.")
