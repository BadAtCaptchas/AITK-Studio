...
try:
    ...
    raise ImportError(
        "Anima requires the Diffusers revision pinned by ai-toolkit. "
        "Reinstall the repository requirements before loading an Anima model."
    ) from exc
...
class AnimaPromptEmbeds(PromptEmbeds):
    ...
    def to(self, *args, **kwargs):
        ...
        self.attention_mask = self._move_token_tensor(
            self.attention_mask, args, kwargs
        )
        self.t5_input_ids = self._move_token_tensor(
            self.t5_input_ids, args, kwargs
        )
        self.t5_attention_mask = self._move_token_tensor(
            self.t5_attention_mask, args, kwargs
        )
        return self
    ...
    def concat_prompt_embeds(
        cls,
        prompt_embeds: list["AnimaPromptEmbeds"],
        padding_side: str = "right",
    ):
        max_qwen_len = max(
            prompt.text_embeds.shape[1] for prompt in prompt_embeds
        )
        max_t5_len = max(
            prompt.t5_input_ids.shape[1] for prompt in prompt_embeds
        )
        return cls(
            qwen_prompt_embeds=torch.cat(
                [
                    cls._pad_3d(
                        prompt.text_embeds, max_qwen_len, padding_side
                    )
            ...
            qwen_attention_mask=torch.cat(
                [
                    cls._pad_2d(
                        prompt.attention_mask, max_qwen_len, padding_side
                    )
                    for prompt in prompt_embeds
                ],
                dim=0,
            ...
            t5_input_ids=torch.cat(
                [
                    cls._pad_2d(
                        prompt.t5_input_ids, max_t5_len, padding_side
                    )
                    for prompt in prompt_embeds
                ],
                dim=0,
            ...
            t5_attention_mask=torch.cat(
                [
                    cls._pad_2d(
                        prompt.t5_attention_mask, max_t5_len, padding_side
                    )
                    for prompt in prompt_embeds
                ],
                dim=0,
...
class AnimaModel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.train_text_conditioner = model_config.model_kwargs.get(
        ...
        self.max_sequence_length = model_config.model_kwargs.get(
    ...
    def _quantize_text_component(self, component, qtype: str, label: str):
        options = dict(self.model_config.quantize_kwargs or {})
        report = quantize(
            component,
            weights=get_qtype(
                qtype,
                kernel=options.get("kernel") or "auto",
                max_workspace_mb=options.get("max_workspace_mb") or 64,
            ),
    ...
    def load_model(self):
        ...
        self.model = AnimaTrainableModel(
    ...
    def get_generation_pipeline(self):
        ...
        # ModularPipeline.set_progress_bar_config only walks one level of
        # sub-blocks, while Anima's tqdm bar is nested two levels deep. The
        # public .blocks property returns a copy, so recurse through _blocks.
    ...
    def _offload_text_encoder(self):
        if (
            ...
            and self.pipeline.text_encoder.device != torch.device("cpu")
        ):
    ...
    def encode_images(
        ...
        if isinstance(images, list):
            images = torch.stack(
                [image.to(device, dtype=dtype) for image in images], dim=0
            )
        else:
    ...
    def _condition_prompt_embeds(
        ...
        return self.trainable_model.text_conditioner(
            source_hidden_states=text_embeddings.text_embeds.to(
                self.device_torch, dtype=dtype
            ),
            target_input_ids=text_embeddings.t5_input_ids.to(self.device_torch),
            target_attention_mask=text_embeddings.t5_attention_mask.to(
                self.device_torch
            ),
            source_attention_mask=text_embeddings.attention_mask.to(
                self.device_torch
            ),
        )
    ...
    def get_noise_prediction(
        ...
        **kwargs,
        ...
        latent_model_input = latent_model_input.unsqueeze(2).to(
            self.device_torch, dtype=self.torch_dtype
        )
        timestep = (
            timestep / self.noise_scheduler.config.num_train_timesteps
        ).to(self.device_torch, self.torch_dtype)
        prompt_embeds = self._condition_prompt_embeds(
            text_embeddings, dtype=self.torch_dtype
        )
    ...
    def _get_qwen_prompt_embeds(self, prompt: List[str]):
        ...
        prompt_embeds = prompt_embeds * conditioner_attention_mask.to(
            prompt_embeds
        ).unsqueeze(-1)
    ...
    def _get_t5_prompt_ids(self, prompt: List[str]):
        ...
        return (
            text_inputs.input_ids.to(self.device_torch),
            text_inputs.attention_mask.to(self.device_torch),
        )
    ...
    def get_prompt_embeds(self, prompt: str) -> AnimaPromptEmbeds:
        ...
        try:
            qwen_prompt_embeds, qwen_attention_mask = (
    ...
    def _strip_ai_toolkit_wrapper_prefix(key: str) -> str:
        ...
        if key.startswith("transformer.text_conditioner."):
            return key.replace(
                "transformer.text_conditioner.", "text_conditioner.", 1
            )
        return key
    ...
    def _add_ai_toolkit_wrapper_prefix(key: str) -> str:
        ...
        if key.startswith("text_conditioner."):
            return key.replace(
                "text_conditioner.", "transformer.text_conditioner.", 1
            )
        return key
    ...
    def _convert_diffusers_lora_key_to_comfy(key: str) -> str:
        ...
        if key.startswith("text_conditioner."):
            return key.replace(
                "text_conditioner.", "diffusion_model.llm_adapter.", 1
            )
    ...
    def convert_lora_weights_before_save(self, state_dict):
        return {
            self._convert_diffusers_lora_key_to_comfy(key): value
    ...
    def convert_lora_weights_before_load(self, state_dict):
        if any(key.startswith("diffusion_model.") for key in state_dict):
            ...
            state_dict = _convert_non_diffusers_anima_lora_to_diffusers(
                state_dict
            )
        return {
            self._add_ai_toolkit_wrapper_prefix(key): value
            for key, value in state_dict.items()
        }
