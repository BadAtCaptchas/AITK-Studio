...
class Attention(torch.nn.Module):
    ...
    def forward(
        ...
        if kv_capture is not None and ref_span is not None:
            # Stash post-RoPE ref K/V so later denoising steps can skip refs.
            kv_capture.append(
                (
...
class SingleStreamDiT(nn.Module):
    ...
    def forward(
        ...
        if reflen > 0 and isolate_refs:
            # Ref queries attend only to ref keys; text + noisy queries still
            # see everything. With t=0 ref modulation, ref states become
            # step-invariant and can be cached during inference.
