...
Supports t2v (t2va), spatial first-frame i2v (fl2va), and vision-token
reference conditioning (ref2va) in training and sampling, with joint audio
when the dataset provides it. Image datasets train as single
...
from toolkit.util.quantize import report_prequantized_ostris_model
...
class MinimaxH3Model(BaseModel):
    ...
    def _validate_prequantized_configuration(self) -> None:
        """Reject modes that would mutate H3's shipped packed checkpoints."""
        if bool(getattr(self.model_config, "layer_offloading", False)):
            raise ValueError(
                "MiniMax-H3 does not support layer_offloading with its packed "
                "ConvRot8/NVFP4 weights; use low_vram whole-component movement"
            )
        if not bool(getattr(self.model_config, "quantize", False)) or str(
            getattr(self.model_config, "qtype", "")
        ).lower() != "convrot8":
            raise ValueError(
                "MiniMax-H3 requires the prequantized ConvRot8 transformer "
                "configuration (quantize=true, qtype='convrot8')"
            )
        if not bool(getattr(self.model_config, "quantize_te", False)) or str(
            getattr(self.model_config, "qtype_te", "")
        ).lower() != "nvfp4":
            raise ValueError(
                "MiniMax-H3 requires the prequantized NVFP4 text encoder "
                "configuration (quantize_te=true, qtype_te='nvfp4')"
            )
        te_path = getattr(self.model_config, "te_name_or_path", None)
        if te_path is not None and os.path.isdir(te_path):
            raise ValueError(
                "MiniMax-H3 currently supports only the prequantized single-file "
                "NVFP4 text encoder, not a dense Transformers directory"
            )
    ...
    def _uses_latent_keyframe_conditioning(self) -> bool:
        """FL checkpoints consume spatial keyframe rows; REF checkpoints
        consume the reference image only through Qwen3-VL vision tokens."""
        return self._partition().startswith("fl2va")

    def _dit_component(self) -> str:
        return f"dit_{self._partition()}"

    @staticmethod
    def _transformer_params_for_state(
        state_dict: dict[str, torch.Tensor],
    ) -> MiniMaxH3TransformerParams:
        """Select the released pruned or non-pruned AdaLN topology from the
        checkpoint itself without relying on a filename convention."""
        params = MiniMaxH3TransformerParams()
        table = state_dict.get("adaln_t_table")
        if table is not None:
            if not isinstance(table, torch.Tensor) or table.ndim != 2:
                raise ValueError("adaln_t_table must be a two-dimensional tensor")
    ...
    def _load_transformer(self) -> MiniMaxH3Transformer:
        ...
        # Pruned checkpoints carry a factored timestep table; non-pruned
        # checkpoints retain the timestep MLP.
        params = self._transformer_params_for_state(state_dict)
        ...
        if not num_quantized:
            raise ValueError(
                "MiniMax-H3 transformer checkpoint contains no ComfyUI "
                "prequantized ConvRot8 layers"
            )
        wrong_backends = sorted(
            {
                str(getattr(module, "ostris_backend_name", ""))
                for module in transformer.modules()
                if isinstance(module, OstrisLinear)
                and getattr(module, "ostris_backend_name", None) != "convrot8"
            }
        )
        if wrong_backends:
            raise ValueError(
                "MiniMax-H3 transformer contains unsupported packed backends: "
                + ", ".join(wrong_backends)
            )
        self.print_and_status_update(
            f" - attached {num_quantized} pre-quantized ConvRot8 layers"
        )
        self.quantization_report = report_prequantized_ostris_model(
            transformer,
            component_label="transformer",
            qtype_label="convrot8",
        )
    ...
    def _load_text_encoder(self):
        ...
        if te_path is not None and os.path.isdir(te_path):
            ...
            if num_quantized == 0 or "nvfp4" not in packed_backends:
                raise ValueError(
                    "MiniMax-H3 text encoder checkpoint contains no "
                    "prequantized NVFP4 layers"
                )
            unsupported_backends = sorted(
                packed_backends.difference({"nvfp4", "convrot8"})
            )
            if unsupported_backends:
                raise ValueError(
                    "MiniMax-H3 text encoder contains unsupported packed backends: "
                    + ", ".join(unsupported_backends)
                )
            self.print_and_status_update(
                f" - attached {num_quantized} pre-quantized NVFP4/int8 layers"
            )
            self.text_encoder_quantization_report = report_prequantized_ostris_model(
                text_encoder,
                component_label="text_encoder",
                qtype_label="nvfp4+convrot8+int8_embedding",
            )
            state_dict = {
    ...
    def load_model(self):
        ...
        self._validate_prequantized_configuration()
        self.print_and_status_update("Loading MiniMax-H3 model")

        transformer = self._load_transformer()

        # Load the assistant as a live frozen adapter; packed weights are never merged.
        ...
        self.print_and_status_update(
            "Text encoder is already NVFP4/int8 quantized; leaving packed weights untouched"
        )
        if self.model_config.low_vram:
            text_encoder.to("cpu")
        else:
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            if raw_audio is not None:
                ...
                # rides along without contributing to the loss. Reuse the
                # same draw for every secondary pass so video comparisons do
                # not change merely because their joint audio context did.
                silence_shape = (
                    batch_size,
                ...
                if (
                    batch is not None
                    and batch.audio_noise is not None
                    and tuple(batch.audio_noise.shape) == silence_shape
                ):
                ...
                audio_rows = sa * audio_noise
                if batch is not None:
                    # Cached audio can outlive a dataset-config change. A
                    # silent clip, disabled audio, or an image batch must not
                    # retain a target/prediction from an earlier pass.
