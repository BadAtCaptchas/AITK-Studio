...
class MiniMaxH3Transformer(nn.Module):
    ...
    def to(self, *args, **kwargs):
        """Move the transformer without flattening its checkpoint dtypes.

        Studio's generic trainer and sampler paths pass their working dtype to
        ``Module.to``. H3 checkpoints are intentionally mixed precision (and
        also contain packed byte buffers), so applying that dtype globally
        corrupts both the fp32 islands and the packed representation. Parse the
        normal ``to`` overloads, discard only their dtype conversion, and keep
        device, non-blocking, and memory-format behavior unchanged.
        """
