...
def run_transformer(
    ...
    **kwargs,
    ...
    if not torch.equal(image_slot_mask, image_slot_mask[:1].expand_as(image_slot_mask)):
        raise ValueError("Every prompt in a batch must use the same reference image slot layout")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for step_index, timestep in enumerate(scheduler.timesteps):
            ...
            preview = None
            if getattr(model, "sample_step_hook", None) is not None:
                # This sampler creates its scheduler inside the call, so report
                # clean latents explicitly for live preview and cancellation.
                preview = latents - scheduler.sigmas[step_index].to(latents) * noise_pred
            ...
            if preview is not None:
                model._emit_sample_step(preview, step_index, len(scheduler.timesteps))
