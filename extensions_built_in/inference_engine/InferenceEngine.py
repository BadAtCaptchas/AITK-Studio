...
The server binds 127.0.0.1 on an ephemeral port and publishes the address to <job_folder>/engine.json; the UI proxies to it.
"""
...
class InferenceEngine(BaseExtensionProcess):
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        training_root = self.get_conf("training_folder", None)
        job_folder = self.engine_config.job_folder or (os.path.join(training_root, self.name) if training_root else None)
        if not job_folder:
            raise ValueError("InferenceEngine requires a job_folder or configured training_folder")
    ...
    def run(self):
        ...
        token = os.environ.get("AITK_ENGINE_TOKEN") or self.engine_config.token or secrets.token_urlsafe(24)
        failure = None
        try:
    ...
    def start_server(self, app, token: str):
        ...
        self.endpoint = {
            "host": host,
            "port": port,
            "attempt_id": os.environ.get("AITK_ATTEMPT_ID"),
            "pid": os.getpid(),
    ...
    def shutdown(self, failure=None):
        ...
        if self.engine is not None:
            try:
                ...
                pass
        try:
            status = "error" if failure else "queued" if reason == "queued" else "stopped"
            self.ui.update_status(status, str(failure) if failure else "Engine queued" if status == "queued" else "Engine stopped")
        finally:
            self.ui.close()

    def _remove_endpoint(self):
        # A stale attempt must never remove the endpoint of a newer process.
        try:
            with open(self.endpoint_path, encoding="utf-8") as handle:
                endpoint = json.load(handle)
            if endpoint.get("pid") == os.getpid() and endpoint.get("attempt_id") == os.environ.get("AITK_ATTEMPT_ID"):
