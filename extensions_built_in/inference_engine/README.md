...
registered model arch (image, video, audio, text). It hot-swaps models per request:
...
From the UI: **Generate > Live inference**, pick a local GPU, **Start engine**. The
  ...
  process:
    - type: InferenceEngine
      ...
      engine:
        ...
        # Supply AITK_ENGINE_TOKEN in the environment for headless API access
...
`<job_folder>/engine.json` (`host`, `port`, `pid`, `job_id`, `attempt_id`). The UI proxies
`/api/inference/*` to it; nothing else is exposed.

## HTTP API

All routes, including `/health`, need the token (`X-Engine-Token: <token>` or
...
Studio derives engine credentials in memory for the active job attempt; endpoint files never contain a token. The UI requires the managed app stack (`npm run dev` or `npm run start`). Live inference currently runs on local GPUs; existing remote training and generation workflows remain available.
