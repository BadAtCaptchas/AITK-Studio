...
class Engine:
    def __init__(
        ...
        self._holder_lock = threading.RLock()
    ...
    def unload(self, wait: bool = True):
        if not self._holder_lock.acquire(blocking=wait):
            raise EngineBusy("busy")
        try:
            self._unload()
        finally:
            self._holder_lock.release()
    ...
    def _build_holder(self, job: GenerationJob, model: dict):
        ...
        holder = ModelClass(
            ...
            dtype=model.get("dtype") or self.dtype,
    ...
    def _run(self, job: GenerationJob):
        ...
        try:
            ...
            before = set(_list_outputs(out_dir, include_text=entry["modality"] == "text"))
            ...
            try:
                ...
                if hasattr(holder, "_status_update_hooks"):
                    try:
                        ...
                        pass
            produced = [p for p in _list_outputs(out_dir, include_text=entry["modality"] == "text") if p not in before]
            if not produced:
...
def _list_outputs(folder: str, include_text: bool = False) -> List[str]:
    return [
        p
        for p in glob.glob(os.path.join(folder, "*"))
        if os.path.isfile(p) and not os.path.basename(p).startswith(".")
        and (include_text or not p.endswith(".txt"))
    ]
