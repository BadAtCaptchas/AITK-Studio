...
from .engine import END, Engine, EngineBusy
...
def create_app(engine: Engine, token: Optional[str] = None) -> FastAPI:
    ...
    async def unload(request: Request):
        ...
        try:
            await asyncio.to_thread(engine.unload, wait=False)
        except EngineBusy:
            raise HTTPException(status_code=409, detail="busy") from None
        return {"ok": True}
    ...
    async def generate(request: Request):
        ...
        if engine.stop_event.is_set():
            raise HTTPException(status_code=503, detail="engine is stopping")
        try:
            body = await request.json()
            model, sample, stream_config = validate_generation(body)
        ...
        if body.get("wait"):
            ...
            try:
                while True:
                    try:
                        frame = await asyncio.to_thread(q.get, True, 1.0)
                    except queue.Empty:
        ...
        job = engine.submit(model, sample, stream_config)
