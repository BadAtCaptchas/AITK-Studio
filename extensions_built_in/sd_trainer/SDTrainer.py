...
class SDTrainer(BaseSDTrainProcess):
        ...
        if loss_target == 'source' or loss_target == 'unaugmented':
            ...
            if dopsd_normal_target is not None:
                ...
                with torch.no_grad():
                    bleed_scale = (
                        loss.detach().mean()
                        / bleed_loss.detach().mean().clamp(min=1e-8)
                    )
                bleed_strength = float(
                    getattr(self.sd, 'dopsd_bleed_strength', 1.0)
                )
    ...
    def train_llm_accumulation(self, batch: DataLoaderBatchDTO):
    ...
    def train_single_accumulation(self, batch: DataLoaderBatchDTO):
        if getattr(self.sd, 'is_llm', False):
            return self.train_llm_accumulation(batch)
        with torch.no_grad():
                ...
                if ((
                    ...
                    with self.timer('prior predict'):
                        ...
                        if is_dopsd:
                            ...
                            prior_embeds_to_use = (
                                batch.dopsd_prompt_embeds.clone()
                                .detach()
