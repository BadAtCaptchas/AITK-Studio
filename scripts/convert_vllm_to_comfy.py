...
linear/embedding markers, not the 3D expert banks â€” the banks are for comfy.
...
class Qwen3OmniHandler(ArchHandler):
    """Qwen3-Omni: keep only the thinker (the VLM â€” talker and code2wav are
...
def quantize_rows(weight: torch.Tensor, rot: int, device):
    ...
    (the trailing 1 is comfy_kitchen's per-channel scale convention â€” its
