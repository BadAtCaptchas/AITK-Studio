...
class GlmImageStaticSupportTest(unittest.TestCase):
    ...
    def test_auto_training_profiles_are_model_scoped(self):
        ...
        self.assertIn("linear: 32", source)
        self.assertIn("linear_alpha: 32", source)
        self.assertIn("linear: 16", source)
        self.assertIn("linear_alpha: 16", source)
        self.assertIn("dropout: 0.05", source)
        self.assertIn("gradient_accumulation: 2", source)
        self.assertIn("lr: 0.000015", source)
        self.assertIn("lr: 0.00001", source)
    ...
    def test_training_phase_editor_filters_profiles_by_model(self):
        ...
        self.assertIn("profile.modelArchs.includes(modelArchName)", editor_source)
        self.assertIn("modelArchs: modelArchName ? [modelArchName] : undefined", editor_source)
