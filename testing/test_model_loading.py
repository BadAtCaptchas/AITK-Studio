...
MODEL_TESTS = {
    ...
    "ltx2.3": {
        # even quantized, the 22B stack does not fit a 32GB card â€” needs the big GPU
    ...
    "qwen_image_2": {
        ...
        # AI Toolkit uses guidance 3; Diffusers defaults to 1. One arch: the control
...
def run_one(
    ...
    if (
        ...
        raise ValueError(
            ...
            f"(arch={getattr(ModelClass, 'arch', None)!r}) â€” registry mismatch"
        )
    ...
    if quant_only and no_sample:
        # load+quantize only â€” no forward at all; catches quantize-time
    ...
    # one_minus_z[-1], which is zero for a single step â€” NaN timesteps and a
...
def write_report(results, path):
    ...
    for r in results:
        if r["status"] != "PASS" or "stages" not in r:
            lines += [f"## {r['arch']} â€” {r['status']}", "", r.get("error", ""), ""]
            continue
        lines += [f"## {r['arch']}", ""]
        lines += [
...
def main():
    ...
    parser.add_argument(
        ...
        "pass, no offload probe â€” just proves each backend loads/quantizes/"
        "runs without errors",
    )
    parser.add_argument(
        "--qtypes",
    ...
    if args.quant_test and args.qtype_override is None:
        ...
        for arch in archs:
            for qt in qtypes:
                ...
                print(
                    f">>> quant-test progress: {i}/{total} â€” elapsed "
    ...
    if args.arch is not None:
        ...
        if result["status"] == "PASS" and "stages" in result:
            ...
            print()
        print(f"[{result['status']}] {args.arch}" + (f" â€” {result.get('error', '')}" if result["status"] != "PASS" else ""))
    ...
    for i, arch in enumerate(MODEL_TESTS, start=1):
        ...
        print(
            f">>> progress: {i}/{total} ({i * 100 // total}%) â€” "
            f"{tally['PASS']} pass, {tally['FAIL']} fail, {tally['SKIP']} skip â€” "
