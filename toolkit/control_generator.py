...
ControlTypes = Literal['depth', 'pose', 'line', 'inpaint', 'mask', 'sapiens2_mask']
...
class ControlGenerator:
    ...
    def preprocess(self, image, control_type):
        # CPU stage. For the bg-remover path this returns a ready-to-run tensor,
        # so the GPU thread does only the model forward.
    ...
    def run_inference(self, payload, control_type):
        # GPU stage. Returns an intermediate result for postprocess().
    ...
    def run_control(self, image, control_type):
        ...
        # resulting PIL image without disk IO.
        device = self.device
        self.ensure_unloaded()
        ...
        elif control_type == 'sapiens2_mask':
