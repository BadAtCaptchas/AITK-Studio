...
into the param's dtype on CUDA (ordinary rounding on CPU). The trainer calls
prepare_gradients() before normalization and clipping; .step() also supports
standalone use. zero_grad() clears private buffers, including on OOM retries.
"""
...
def _get_kernel():
    ...
    def _rot256(x, hb, R: tl.constexpr):
        # This also rotates floating gradient increments, not just integer
        # codes. Plain TF32 truncates those and can change low-bit moment codes.
        x3 = tl.reshape(x, (R, 16, 16))
        z = tl.dot(x3, hb, input_precision="tf32x3")
        y3 = tl.dot(hb, z, input_precision="tf32x3")
...
class AdamConvRot(torch.optim.Optimizer):
    ...
    @torch.no_grad()
    def _copy_parameter(self, target, source):
        if not self.stochastic_rounding or not target.is_cuda:
            target.copy_(source)
        elif target.dtype == torch.float16:
            # FP16 spacing depends on magnitude. A fixed fractional grid loses
            # tiny gradients and rounds large updates deterministically.
            _, exponent = torch.frexp(source)
            ulp = torch.exp2((exponent - 11).clamp_min(-24).float())
            target.copy_(torch.floor(source / ulp + torch.rand_like(source)) * ulp)
        else:
            copy_stochastic(target, source)

    @torch.no_grad()
    def _accumulate_grad(self, p):
        accum = getattr(p, '_accum_grad', None)
        if accum is None:
            p._accum_grad = p.grad.clone()
        else:
            value = accum.float() + p.grad.float()
            self._copy_parameter(accum, value)
        p.grad = None
    ...
    def prepare_gradients(self):
        """Expose accumulated buffers before trainer normalization and clipping."""
        for group in self.param_groups:
            for p in group['params']:
                accum = getattr(p, '_accum_grad', None)
    ...
    def zero_grad(self, set_to_none=True):
        # OOM retries must never retain gradients from a discarded attempt.
        for group in self.param_groups:
            for p in group['params']:
    ...
    def _step_quant(self, p, state, group, bc1, bc2, seed, sanitize=False):
        use_triton = (
            ...
            and torch.cuda.get_device_capability(p.device)[0] >= 8
