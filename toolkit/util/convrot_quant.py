...
def get_convrot_quantizer(
    qtype: str, *, kernel: str = "auto", max_workspace_mb: int = 64
):
    if qtype == "convrot4":
        return ConvRotQuantizer(
            rot_size=256, kernel=kernel, max_workspace_mb=max_workspace_mb
        )
    if qtype == "convrot8":
...
def _workspace_rows(
    total_rows: int, columns: int, dtype: torch.dtype, max_workspace_mb: int
) -> int:
    """Conservative row count for rotation/quantization work buffers."""
...
def _validate_runtime_options(kernel: str, max_workspace_mb: int) -> None:
    if kernel not in {"auto", "triton", "torch"}:
        raise ValueError("kernel must be one of: auto, triton, torch")
    if not isinstance(max_workspace_mb, int) or isinstance(max_workspace_mb, bool):
        raise TypeError("max_workspace_mb must be a positive integer")
...
def regular_hadamard(rot_size: int, device, dtype=torch.bfloat16) -> torch.Tensor:
    ...
    def build():
        # fp32, not fp64: entries stay exactly +/-1 through the Kronecker
        # products and rot_size is a power of four, so normalization by its
        # power-of-two square root is exact. Besides avoiding an unnecessary
        # conversion, this keeps cache misses usable on devices without fp64.
        r4 = torch.tensor(
...
def _e4m3_triton_ok(device) -> bool:
    """Return whether Triton can compile e4m3 kernels for ``device``.

    Triton's fp8e4nv support requires SM 8.9 or newer. Older CUDA devices fail
    at kernel compilation, so they must use the numerically equivalent torch
    fallback even when Triton itself is installed.
    """
    key = str(device)
    if key not in _e4m3_triton_ok_cache:
        try:
            _e4m3_triton_ok_cache[key] = (
...
def _import_triton():
    """Import Triton lazily and publish its names for older releases.

    Older Triton versions resolve free variables in a JIT kernel exclusively
    through ``fn.__globals__``. Kernels defined by the builders below therefore
    need ``triton`` and ``tl`` at module scope as well as in their closure.
    """
...
def _use_triton(kernel: str) -> bool:
    ...
    if not available and kernel == "triton" and kernel not in _triton_fallback_warned:
        ...
        warnings.warn(
            "ConvRot requested the Triton kernel, but Triton is unavailable; "
            "falling back to the bounded torch implementation.",
            RuntimeWarning,
            stacklevel=2,
        )
    return available
...
class ConvRotQuantizer(OstrisQuantizer):
    ...
    def __init__(
        ...
        *,
        ...
        _validate_runtime_options(kernel, max_workspace_mb)
    ...
    def _quantize_weight(self, weight: torch.Tensor, rot: int):
        ...
        packed = torch.empty(
            (rows, columns // 2), device=weight.device, dtype=torch.uint8
        )
        scales = torch.empty(
            (rows, columns // BLOCK),
        ...
        for start, end in _row_chunks(rows, rows_per_chunk):
            rotated = rotate(weight[start:end], rot)
            packed_chunk, scales_chunk, _ = quantize_nvfp4(
                rotated, pts=pts, optimize_scales=True
            )
    ...
    def quantize_(self, module: torch.nn.Linear, weight_fp32: torch.Tensor) -> None:
        ...
        packed, scales, pts = self._quantize_weight(weight_fp32, rot)
    ...
    def _decode_rotated_rows(
        self, module, start: int, end: int, dtype: torch.dtype
    ) -> torch.Tensor:
    ...
    def dequantize_to(
        self, module, device: torch.device, dtype: torch.dtype
    ) -> torch.Tensor:
    ...
    def _dequantize_rotated(self, module, dtype: torch.dtype) -> torch.Tensor:
        rows_per_chunk = self._runtime_rows(
            module.out_features, module.in_features, dtype
        )
        chunks = [
    ...
    def dequantize(self, module) -> torch.Tensor:
        return self.dequantize_to(module, module.cr_qdata.device, torch.float32)
    ...
    def requantize_codes_(self, module, fp_weight: torch.Tensor) -> None:
        ...
        rows, K = w.shape
        edges = _cached(
            _edges_cache, str(w.device),
            lambda: torch.tensor(_E2M1_EDGES, device=w.device),
        )
        packed = torch.empty_like(module.cr_qdata)
        rows_per_chunk = self._runtime_rows(rows, K, torch.float32)
        stored_scales = module.cr_scales.view(torch.float8_e4m3fn).float()
        stored_scales = stored_scales * self._pts(module)
        for start, end in _row_chunks(rows, rows_per_chunk):
            w_rot = rotate(w[start:end], module.cr_rot_size)
            denom = stored_scales[start:end].unsqueeze(-1)
            safe = torch.where(denom > 0, denom, torch.ones_like(denom))
            z = (
                w_rot.view(end - start, K // BLOCK, BLOCK) / safe
            ).clamp(-F4_MAX, F4_MAX).reshape(end - start, K)
            mag = torch.bucketize(z.abs(), edges).to(torch.uint8)
    ...
    def requantize_(self, module, fp_weight: torch.Tensor) -> None:
        w = fp_weight.to(device=module.cr_qdata.device, dtype=torch.float32)
        packed, scales, pts = self._quantize_weight(w, module.cr_rot_size)
        module.cr_qdata = packed
    ...
    def _fake_quantize_activation(self, value: torch.Tensor) -> torch.Tensor:
        ...
        for start, end in _row_chunks(rows, rows_per_chunk):
            ...
            output[start:end].copy_(
                dequantize_nvfp4(
                    packed, scales, pts, end - start, columns, value.dtype
                )
            )
        return output
    ...
    def forward(self, module, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.requires_grad:
            ...
            if self.kernel != "torch" and _fp4_gemm_supported(x.device):
            ...
            x2d = rotate(x, rot).reshape(-1, in_f)
            with torch.no_grad():
                x_dq = self._fake_quantize_activation(x2d.detach())
        ...
        if self.kernel != "torch" and _fp4_gemm_supported(x.device):
            ...
            try:
                ...
                if key not in _runtime_fast_warned:
                    ...
                    warnings.warn(
                        f"ConvRot4 hardware path failed ({type(error).__name__}); "
        ...
        out = self._fallback_forward(module, rotate(x, rot).reshape(-1, in_f))
        return out.reshape(*x.shape[:-1], out_f)
...
class ConvRotInt8Quantizer(OstrisQuantizer):
    ...
    def quantize_(self, module: torch.nn.Linear, weight_fp32: torch.Tensor) -> None:
        ...
        q = torch.empty_like(weight_fp32, dtype=torch.int8)
        scales = torch.empty(
            module.out_features, device=weight_fp32.device, dtype=torch.float32
        )
        for start, end in _row_chunks(module.out_features, rows_per_chunk):
            q_chunk, scales_chunk = quantize_int8_rows(
                rotate(weight_fp32[start:end], rot)
            )
            q[start:end].copy_(q_chunk)
            scales[start:end].copy_(scales_chunk)
    ...
    def _qdata_rows(self, module, start: int, end: int) -> torch.Tensor:
        return module.cr8_qdata[start:end]
    ...
    def _decode_rotated_rows(
        ...
        scales = self._scales(module)[start:end].unsqueeze(1)
        return (self._qdata_rows(module, start, end).float() * scales).to(dtype)
    ...
    def dequantize(self, module) -> torch.Tensor:
        return self.dequantize_to(
            module, self._scales_u8(module).device, torch.float32
        )
    ...
    def requantize_(self, module, fp_weight: torch.Tensor) -> None:
        ...
        scales = torch.empty(module.out_features, device=w.device, dtype=torch.float32)
        for start, end in _row_chunks(module.out_features, rows_per_chunk):
            q_chunk, scales_chunk = quantize_int8_rows(
                rotate(w[start:end], module.cr8_rot_size)
            )
            q[start:end].copy_(q_chunk)
            scales[start:end].copy_(scales_chunk)
        module.cr8_qdata = q
    ...
    def _fake_quantize_activation(self, value: torch.Tensor) -> torch.Tensor:
        ...
        rows_per_chunk = self._runtime_rows(rows, columns, torch.float32)
        for start, end in _row_chunks(rows, rows_per_chunk):
            q, scales = quantize_int8_rows(value[start:end], self.act_qmax)
            output[start:end].copy_(
                (q.float() * scales.unsqueeze(1)).to(value.dtype)
            )
        return output
    ...
    def requantize_codes_(self, module, fp_weight: torch.Tensor) -> None:
        ...
        w = fp_weight.to(device=module.cr8_qdata.device, dtype=torch.float32)
        qdata = torch.empty_like(module.cr8_qdata)
        rows_per_chunk = self._runtime_rows(
            module.out_features, module.in_features, torch.float32
        )
    ...
    def forward(self, module, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.kernel != "torch" and _int8_gemm_supported(x.device):
            ...
            try:
                ...
                key = (f"convrotint{getattr(self, 'bits', 8)}", type(error).__name__)
                if key not in _runtime_fast_warned:
                    _runtime_fast_warned.add(key)
                    warnings.warn(
                        f"ConvRot int hardware path failed ({type(error).__name__}); "
                        "using bounded fallback",
                        RuntimeWarning,
                        stacklevel=2,
                    )
...
class ConvRotIntNQuantizer(ConvRotInt8Quantizer):
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(bits, int) or isinstance(bits, bool) or not 2 <= bits <= 8:
    ...
    def quantize_(self, module: torch.nn.Linear, weight_fp32: torch.Tensor) -> None:
        ...
        module.convrot_kernel = self.kernel
        module.convrot_max_workspace_mb = self.max_workspace_mb
        module.convrot_packed_layout = f"int{self.bits}_bitpacked_v1"
    ...
    def _qdata_rows(self, module, start: int, end: int) -> torch.Tensor:
        ...
        if gratio is not None:
            return _intn_unpack_grouped_op(
                module.crn_qdata[start:end],
                gratio.view(torch.float32)[start:end],
                module.crn_bits,
                end - start,
                module.in_features,
            )
    ...
    def requantize_(self, module, fp_weight: torch.Tensor) -> None:
        ...
        for start, end in _row_chunks(module.out_features, rows_per_chunk):
            packed, scales, gratio = self._quantize_rotated(
                rotate(w[start:end], module.crn_rot_size), module.in_features
            )
            packed_chunks.append(packed)
            scale_chunks.append(scales)
            if gratio is not None:
    ...
    def _codes_on_stored_scales(
        ...
        if end is None:
            end = start + w_rot.shape[0]
        qmax = (1 << (module.crn_bits - 1)) - 1
        rs = self._scales(module)[start:end].unsqueeze(1)
        wf = w_rot.float()
        gratio = getattr(module, "crn_gratio", None)
        if gratio is None:
            return torch.round(wf / rs).clamp_(-qmax, qmax)
        gratio = gratio.view(torch.float32)[start:end]
    ...
    def requantize_codes_(self, module, fp_weight: torch.Tensor) -> None:
        ...
        for start, end in _row_chunks(module.out_features, rows_per_chunk):
            codes = self._codes_on_stored_scales(
                module,
                rotate(w[start:end], self._rot(module)),
                start,
                end,
            )
            packed_chunks.append(self._pack(codes.to(torch.int8)))
        module.crn_qdata = torch.cat(packed_chunks, dim=0)
...
class ConvRotBitNetQuantizer(ConvRotIntNQuantizer):
    ...
    def quantize_(self, module: torch.nn.Linear, weight_fp32: torch.Tensor) -> None:
        super().quantize_(module, weight_fp32)
        module.convrot_packed_layout = "ternary_base3_v1"
    ...
    def _qdata_rows(self, module, start: int, end: int) -> torch.Tensor:
        return _bitnet_unpack_op(
            module.crn_qdata[start:end],
            module.crn_gratio.view(torch.float32)[start:end],
            end - start,
            module.in_features,
        )
...
class ConvRotComfyW4A4Quantizer(ConvRotIntNQuantizer):
    ...
    def quantize_(self, module: torch.nn.Linear, weight_fp32: torch.Tensor) -> None:
        super().quantize_(module, weight_fp32)
        module.convrot_packed_layout = "comfy_w4a4_int4_v1"
