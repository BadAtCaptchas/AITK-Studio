...
def unswizzle_nvfp4_scales(scales: torch.Tensor, rows: int, cols: int) -> torch.Tensor:
    ...
    if rows <= 0 or cols <= 0:
        raise ValueError(f"nvfp4 scale dimensions must be positive, got {rows}x{cols}")
    ...
    if scales.numel() != expected:
        raise ValueError(
            "nvfp4 swizzled scale tensor has "
            f"{scales.numel()} values, expected {expected} for {rows}x{cols}"
        )
...
class Nvfp4Quantizer(OstrisQuantizer):
    ...
    def attach_(
        ...
        pre_scale_buffer = (
            ...
            else pre_scale.detach().float().clone().contiguous().view(torch.uint8)
        )
        module.register_buffer(
            "nv4_pre_scale",
            pre_scale_buffer,
    ...
    def _pre_scale(module) -> Optional[torch.Tensor]:
        ...
        return None if buf is None or buf.numel() == 0 else buf.view(torch.float32)
    ...
    def dequantize(self, module: OstrisLinear) -> torch.Tensor:
        """Reconstruct the original-basis weight expected by Studio callers."""
    ...
    def requantize_(self, module: OstrisLinear, fp_weight: torch.Tensor) -> None:
        raise RuntimeError(
            "nvfp4 packed weights cannot be merged or requantized; keep adapters live"
        )
