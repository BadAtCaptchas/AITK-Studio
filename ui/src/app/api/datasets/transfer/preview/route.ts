import { NextResponse } from 'next/server';
import { DatasetScopeError } from '@/server/datasetScope';
import { DatasetTransferError, previewDatasetTransfer } from '@/server/datasetTransfer';

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export async function POST(request: Request) {
  try {
    const body: unknown = await request.json().catch(() => null);
    const payload = isRecord(body) ? body : {};
    return NextResponse.json(
      await previewDatasetTransfer({
        sourceProjectID: payload.source_project_id,
        destinationProjectID: payload.destination_project_id,
        operation: payload.operation,
        all: payload.all,
        datasetNames: payload.dataset_names,
      }),
    );
  } catch (error) {
    const status =
      error instanceof DatasetTransferError || error instanceof DatasetScopeError
        ? error.status
        : typeof (error as { status?: unknown })?.status === 'number'
          ? (error as { status: number }).status
          : 400;
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to preview dataset transfer' }, { status });
  }
}
