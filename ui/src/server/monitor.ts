...
class SystemMonitor {
  ...
  private async sampleCpu(): Promise<CpuInfo> {
    ...
    if (this.isMac) {
      try {
        ...
        const ramData: unknown = ms.getRAMUsageSync();
        const cpuData: unknown = ms.getCpuDataSync();
        return {
          name: cpuStatic.name,
          cores: cpuStatic.cores,
          temperature: numericField(cpuData, 'temperature'),
          totalMemory: numericField(ramData, 'total') / (1024 * 1024),
          availableMemory: numericField(ramData, 'free') / (1024 * 1024),
          freeMemory: numericField(ramData, 'free') / (1024 * 1024),
  ...
  private sampleMacGpu(): GPUApiResponse {
    ...
    if (ms) {
      try {
        const gpuData: unknown = ms.getGpuDataSync();
        temperature = numericField(gpuData, 'temperature');
        gpuLoad = numericField(gpuData, 'usage');
      } catch {
        // ignore
      }
      try {
        const fanData = asRecord(ms.getFanDataSync());
        const fanKeys = Object.keys(fanData);
        if (fanKeys.length > 0) {
          fanSpeed = numericField(fanData[fanKeys[0]], 'rpm');
        }
      } catch {
        // ignore
      }
      try {
        const powerData: unknown = ms.getPowerDataSync();
        powerDraw = numericField(powerData, 'gpu');
      } catch {
        // ignore
      }
      try {
        const ramData: unknown = ms.getRAMUsageSync();
        memUsed = numericField(ramData, 'used') / (1024 * 1024);
        memTotal = numericField(ramData, 'total') / (1024 * 1024);
      } catch {
        // ignore
      }
    }
