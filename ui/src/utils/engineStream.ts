...
export async function readEngineFrames(
  ...
  const parse = () => {
    ...
    while (true) {
      ...
      if (hlen > 1024 * 1024) throw new Error('Engine frame header too large');
      ...
      if (!Number.isSafeInteger(plen) || plen > 64 * 1024 * 1024) throw new Error('Engine frame payload too large');
      ...
      if (
        ...
        throw new Error('Invalid engine frame');
  ...
  try {
    while (true) {
      ...
      if (done) {
        if (buf.length) throw new Error('Truncated engine stream');
        break;
      }
...
export function payloadToFloat32(header: Record<string, unknown>, payload: Uint8Array): Float32Array {
  const dtype = header.dtype;
  const n = latentShape(header).reduce((a, b) => a * b, 1);
  if (payload.byteLength !== n * (dtype === 'float32' ? 4 : 2)) throw new Error('Invalid latent payload size');
  if (dtype === 'float32') {
 ...
 */
export function latentToImage(
  header: Record<string, unknown>,
  data: Float32Array,
  preview: PreviewInfo | null,
): LatentImage | null {
  const shape = latentShape(header);
...
function latentShape(header: Record<string, unknown>): number[] {
  if (
    !Array.isArray(header.shape) ||
    header.shape.length > 5 ||
    header.shape.length < 3 ||
    !header.shape.every((v: unknown): v is number => typeof v === 'number' && Number.isInteger(v) && v > 0)
  )
    throw new Error('Invalid latent shape');
  const count = header.shape.reduce((a, b) => a * b, 1);
  if (!Number.isSafeInteger(count) || count > 16 * 1024 * 1024) throw new Error('Latent tensor too large');
  return header.shape;
}
export function parsePreview(value: unknown): PreviewInfo | null {
  ...
  if (
    ...
    typeof v.spatial !== 'number' ||
    typeof v.temporal !== 'number' ||
    !(v.reshape === null || typeof v.reshape === 'string') ||
    !Array.isArray(v.factors) ||
    !v.factors.every(
      row => Array.isArray(row) && row.length === 3 && row.every(n => typeof n === 'number' && Number.isFinite(n)),
    ) ||
    !(
      v.bias === null ||
      (Array.isArray(v.bias) && v.bias.length === 3 && v.bias.every(n => typeof n === 'number' && Number.isFinite(n)))
    )
  )
    return null;
  if (v.factors.length !== (v.reshape === 'flux2_2x2' ? 32 : v.channels)) return null;
  return {
    format: v.format,
    channels: v.channels,
    dims: v.dims,
